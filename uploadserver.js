...
if (cluster.isMaster) {
    ...
    function randomString(callback) {
        ...
        if (sqlConnection.state != "disconnected") {
            sqlConnection.query("SELECT * FROM ?? WHERE filename = ?", [config.mysql_table, buildStr], function (err, results, fields) { //Although it is more likely, to win the lottery jackpot TWO times in a ROW, but hey: We must bear that in mind ^^
                if (!err) {
                    if (!(results.length > 0)) {
                        callback(buildStr)
                    } else {
    ...
    sqlConnection.connect(function (err) {
        if (!err) {
            ...
            app.post('/upload', function (req, res) {
                ...
                randomString(function (randomStr) {
                    ...
                    req.files.fileUpload.mv(config.uploadDir + newFilename, function (err) {
                        if (err) {
                            ...
                            if (sqlConnection.state != "disconnected") {
                                sqlConnection.query("INSERT INTO ?? (filename, orgfilename, activecdn, hashmd5, mime) VALUES (?, ?, ?, ?, ?)", [config.mysql_table, newFilename, req.files.fileUpload.name, config.webservClusterDomainName, md5File.sync(config.uploadDir + newFilename), req.files.fileUpload.mimetype],
